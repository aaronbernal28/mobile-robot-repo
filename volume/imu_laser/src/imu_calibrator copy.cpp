...
void robmovil_ekf::IMUCalibrator::calculate_bias(){
  ...
  if (calibration_data_.empty()) {
    RCLCPP_WARN(this->get_logger(), "No calibration data collected, bias set to zero");
  } else {
    for (size_t i = 0; i < calibration_data_.size(); ++i) {
      bias_ = bias_ + calibration_data_[i];
  tf2::Quaternion delta_q;
  // No rotation correction available at calibration time, use identity quaternion
  delta_q.setValue(0.0, 0.0, 0.0, 1.0);
  }
  }

  tf2::Quaternion delta_q;
  bias_ = bias_ / calibration_data_.size();  // Calculate average bias
  delta_q.setRPY(bias_.getX(), bias_.getY(), bias_.getZ());
  
  orientacion_estimada_ = orientacion_estimada_ * delta_q;
